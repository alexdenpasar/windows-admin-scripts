...
$s1 = New-PSSession -ComputerName $domainServer
...
#[string]$fcs = Get-Content \\$domainServer\AD$\TMP\$env:UserName".txt" | ForEach-Object { $_ -replace "@{Name=", "" } | Set-Content $ftmp2
[string]$fcs = Get-Content \\$domainServer\AD$\TMP\$env:UserName".txt"
$fcs = $fcs -replace "}", ""
$fcs = $fcs -replace "@{Name=", ""
$fcs = $fcs -replace " ", "_"
$fcs | Set-Content \\$domainServer\AD$\TMP\$env:UserName".txt"
